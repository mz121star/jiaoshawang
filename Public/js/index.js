...
var jsindex = new function() {
		...
		this.shopmore = function() {
			...
			kj.ajax.get(url,function(data){
				...
				if(jsindex.shoppages == 0) {
					objul.innerHTML += '<li class="xnone">暂无</li>';
				}
			});
		}
		...
		this.sort_init = function() {
			...
			if(this.sortval == 1) {
				arr[ii].innerHTML += "<img src='" + this.cssdir + "images/sort_up.png'>";
			} else {
				arr[ii].innerHTML += "<img src='" + this.cssdir + "images/sort_down.png'>";
			}
		}
		//设置排序
		this.sort = function(val) {
			...
			if(val == this.sortby) {
				...
				obj[0].innerHTML = (sortval==1) ? str.substr(0,2) + "<img src='" + this.cssdir + "images/sort_up.png'>" : str.substr(0,2) + "<img src='" + this.cssdir + "images/sort_down.png'>";
			} else {
		...
		this.showstate = function() {
			var val = kj.obj("#id_chk_state").checked;
			var x = (val) ? 'none' : '';
			var obj = kj.parent("#id_shopall .state_bg3","table");
			kj.set(obj,"style.display" , x);
			var arr = kj.obj("#id_shopall .state_bg2");
			if(arr) {
				obj = kj.parent(arr,"table");
				if(obj) kj.set(obj,"style.display" , x);
			}
			arr = kj.obj("#id_shopall table");
			var ii = 0;
		...
		this.search = function(key) {
			...
			kj.ajax.get(kj.cfg('baseurl') + '/index.php?app_act=area_search&skey='+key,function(data) {
				...
				for(var i = 0 ; i < arr.length ; i++ ) {
					if(arr[i].type=='shop') {
						...
						html_shop += "<a href='" + kj.cfg('baseurl') + "/index.php?app_act=shop&id=" + arr[i].shop_id + "'>" + arr[i].shop_name + area + "</a>";
					} else if(arr[i].type=='menu') {
						html_menu += '<a href="' + kj.cfg('baseurl') + '/index.php?app_act=shop&id=' + arr[i].shop_id + '"><span class="x1">' + arr[i].menu_title + "</span><span class='x2'>" + arr[i].shop_name + "</span><span class='x3'>￥" +  arr[i].menu_price + "</span></a>";
					} else {
