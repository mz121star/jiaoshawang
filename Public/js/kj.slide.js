...
kj.slide = new function(){
	...
	this.init = function() {
		...
		for(i = 0; i < arr.length;i++ ) {
			...
			for( j = 0 ; j < arr2.length ; j++ ) {
				...
				tit = kj.getAttribute(arr2[j],'mytit');
				if(src!=null) list1[list1.length] = {'src':src , 'url' : url , 'tit' : tit};
			}
			this.stop[i] = false;
			this.index[i] = 0;
	...
	this.scroll = function(ii) {
		if(this.stop[ii] == false) {
			...
			var arr = kj.obj(".kj_slide");
			var url = this.list[ii][this.index[ii]].url;
			if(url == '') url ="javascript:void(0);"
			kj.set(kj.obj(".xpic img" , arr[ii]),'src',this.list[ii][this.index[ii]].src);
			kj.set(kj.obj("a" , arr[ii]),'href',url);
			kj.set(kj.obj(".xbtn .xtxt" , arr[ii]),'innerHTML',this.list[ii][this.index[ii]].tit);
